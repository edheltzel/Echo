...
export class PlayQueue<T> {
  ...
  /**
   * Resolves once the queue is empty AND no job is in flight. Test seam for
   * "everything accepted so far has settled" (the daemon's drainNotifications
   * delegates here). Resolves immediately when already idle.
   */
  drain(): Promise<void> {
    if (this.queue.length === 0 && this.inFlightSince === null) {
  ...
  private settleDrainIfIdle(): void {
    if (this.queue.length === 0 && this.inFlightSince === null && this.drainWaiters.length > 0) {
