...
| `ECHO_DAEMON_URL` | derived from `ECHO_NOTIFY_URL`, else `http://localhost:8888` | **Adapter-side.** Origin of the daemon an adapter talks to. Sets both `POST /notify` and `GET /voices` at once, so pointing a host at a second instance (e.g. an isolated test daemon) is one variable. Resolved in `shared/daemon-endpoints.ts` |
