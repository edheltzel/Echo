...
| Voice ask (coordinator) | ECHO_CONVERSE_PORT, ECHO_CONVERSE_URL, ECHO_CONVERSE_BOOKING_LOCK, ECHO_CONVERSE_LOG_PATH | 32468 (keypad ECHOV; core keeps 3246), http://localhost:32468, ~/.local/state/echo/converse/booking.lock, ~/Library/Logs/echo-converse.log (where an auto-started coordinator's output lands). The lease has no key: it is derived from the capture and transcription caps below |
