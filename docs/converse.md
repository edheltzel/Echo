...
taken). `core_muted` covers speaker mute (`tts`/`all`, the question would not be heard) and
microphone mute (`mic`/`all`, capture will not start). Every refusal releases the booking, `404 unknown_turn` included: the turn
