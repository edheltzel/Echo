...
ECHO_CONFIG_FILE="$HOME/.config/echo/config.json"
...
if [ "$CHECK_ONLY" -eq 1 ]; then
  ...
  if [ -f "$ECHO_CONFIG_FILE" ]; then
    echo "= would preserve persona config: $ECHO_CONFIG_FILE"
  fi
  exit 0
fi
...
if [ -f "$ECHO_CONFIG_FILE" ]; then
  echo "Persona config preserved at $ECHO_CONFIG_FILE"
fi
