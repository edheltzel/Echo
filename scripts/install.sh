...
usage() {
  cat <<EOF
Usage: scripts/install.sh [--adapter none|claudecode|jcode|grok|mcp|pi|omp] [--check]
...
case "$ADAPTER" in
  none|claudecode|jcode|grok|mcp|pi|omp) ;;
  *)
...
check_installation() {
  ...
  local adapter
  if ! skip_workspace_link; then
    for adapter in claudecode jcode grok omp pi; do
